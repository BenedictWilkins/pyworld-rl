...
dt = str(datetime.datetime.now()).split('.')[0].replace(' ','_').replace('-','_').replace(':','_')
GEN_PATH = "models/gen" + dt + ".pt"
DIS_PATH = "models/disc" + dt + ".pt"
STOP = 100000
...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        return conv_out.view(-1, 1).squeeze(dim=1) #removes all dimensions of dim = 1 (A x B x 1) -> (A x B)
...
if __name__ == "__main__":
    ...
    for batch_v in iterate_batches(envs):
        ...
        if iter_no % SAVE_IMAGE_EVERY_ITER == 0:    
            ...
            gen_img  = vutils.make_grid(gen_output_v.data[:64], normalize=True)
        ...
        iter_no += 1
        if iter_no > STOP:
            torch.save({"model":net_discr.state_dict(), "opt":dis_optimizer.state_dict()}, DIS_PATH)
            torch.save({"model":net_gener.state_dict(), "opt":gen_optimizer.state_dict()}, GEN_PATH)
            print("DONE")
            break
